# Sanal Ekonomi V13.1 — Auth Fix (Validated)
...
V13.1 additional fix: legacy `users.password` schemas are migrated/handled so register/login do not fail when the database was created by an older version. The login/register UI now shows immediate progress and clear timeout/network errors.
