...
let HAS_LEGACY_PASSWORD_COLUMN = false;
...
async function initDb() {
  ...
  // IMPORTANT: older deployments may have a users table without password_hash.
  // Keep this migration backward-compatible so authentication never depends on
  ...
  // If an older schema used a legacy password column, copy it forward when
  // password_hash is empty. This is safe for bcrypt hashes and preserves the
  // existing account instead of forcing the player to re-register.
  const legacyPasswordColumn = await q(`
    SELECT 1
    FROM information_schema.columns
    WHERE table_schema='public' AND table_name='users' AND column_name='password'
    LIMIT 1
  `);
  HAS_LEGACY_PASSWORD_COLUMN = legacyPasswordColumn.rows.length > 0;
  if (HAS_LEGACY_PASSWORD_COLUMN) {
    await q(`
      UPDATE users
      SET password_hash = password
      WHERE COALESCE(password_hash,'')='' AND password IS NOT NULL
    `);
  }
...
app.post("/register", async (req, reply) => {
  try {
    ...
    try {
      ...
      const insertSql = HAS_LEGACY_PASSWORD_COLUMN
        ? `
          INSERT INTO users(
            username,password_hash,password,scoin,iron,steel,wood,stone,energy,machines,land,mine_level,mines
          )
          VALUES($1,$2,$2,$3,$4,0,$5,$6,$7,0,$8,1,0)
        ...
        : `
          ...
          RETURNING id,username,scoin,iron,steel,wood,stone,energy,machines,land,mine_level
        `;
      const insertParams = HAS_LEGACY_PASSWORD_COLUMN
        ? [clean, hash, appConfig.starter.scoin, appConfig.starter.iron, appConfig.starter.wood, appConfig.starter.stone, appConfig.starter.energy, appConfig.starter.land]
        : [clean, hash, appConfig.starter.scoin, appConfig.starter.iron, appConfig.starter.wood, appConfig.starter.stone, appConfig.starter.energy, appConfig.starter.land];
...
app.post("/login", async (req, reply) => {
  try {
    ...
    const loginSql = HAS_LEGACY_PASSWORD_COLUMN
      ? `SELECT id,username,password_hash,password,scoin,iron,steel,wood,stone,energy,machines,land,mine_level
         FROM users WHERE LOWER(username)=LOWER($1) LIMIT 1`
      : `SELECT id,username,password_hash,scoin,iron,steel,wood,stone,energy,machines,land,mine_level
         FROM users WHERE LOWER(username)=LOWER($1) LIMIT 1`;

    const r = await q(loginSql,[clean]);

    const u = r.rows[0];

    const storedHash = u && (u.password_hash || (HAS_LEGACY_PASSWORD_COLUMN ? u.password : null));
    if (!u || !storedHash || !(await bcrypt.compare(password, storedHash))) {
...
async function api(path, options={}){
  ...
  try{
    ...
    if(e?.name==="AbortError") throw new Error("Sunucu yanıt vermedi (30 sn zaman aşımı). Render Logs kontrol edilmeli.");
    throw new Error("Sunucuya bağlanılamadı: "+(e?.message||e));
  }finally{
...
async function doRegister(){
  ...
  msg.textContent="Kayıt işlemi başlatılıyor...";

  if(username.length<3){msg.textContent="Kullanıcı adı en az 3 karakter olmalı.";return}
  ...
  try{
    ...
    token=d.token;
    msg.textContent="Kayıt başarılı. Oyun yükleniyor...";
    await openGame();
  }catch(e){msg.textContent="Kayıt hatası: "+(e?.message||e);}
}
...
async function doLogin(){
  ...
  const msg=document.getElementById("authMsg");
  msg.textContent="Giriş kontrol ediliyor...";

  try{
    const d=await api("/login",{method:"POST",body:JSON.stringify({username,password})});
    token=d.token;
    msg.textContent="Giriş başarılı. Oyun yükleniyor...";
    await openGame();
  }catch(e){msg.textContent="Giriş hatası: "+(e?.message||e);}
}
...
async function openGame(){
  ...
  try{
    ...
    msg.textContent="Oyun verileri yüklenemedi: "+(e?.message||e);
    token="";
  }
}
