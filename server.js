...
async function repairPlayerIfEmpty(id,client=pool){
  ...
  if(!u) return false;
  const allZero = [u.scoin,u.iron,u.wood,u.stone,u.energy,u.steel,u.machines].every(v=>num(v)===0);
  const noLand = Number(u.land||0)===0;
  if(!allZero || !noLand) return false;
  ...
  const tx=await q(`SELECT 1 FROM transactions WHERE user_id=$1 AND type='REPAIR' LIMIT 1`,[id],client);
  if(!tx.rows.length){
    await q(`INSERT INTO transactions(user_id,type,amount,description) VALUES($1,'REPAIR',0,'Boş başlangıç hesabı otomatik olarak düzeltildi')`,[id]);
  }
  return true;
}
...
app.get("/health",async()=>({ok:true,service:"sanal-ekonomi",version:"17.0.0",currency:"SCoin"}));
