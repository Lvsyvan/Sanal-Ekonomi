...
async function initDb(){
  ...
  // Repair legacy/broken player rows. Land can never legitimately be 0 in this game.
  // Therefore any account with land <= 0 gets a deterministic starter package.
  // This also fixes accounts that already have partial/empty building rows.
  const broken=await q(`
    SELECT u.id
    FROM users u
    WHERE COALESCE(u.land,0) <= 0
  `);
  for(const row of broken.rows){
    await q(`
      UPDATE users SET
        scoin=$1,iron=$2,steel=$3,wood=$4,stone=$5,energy=$6,machines=$7,
        land=$8,mine_level=$9,mines=0,production_updated_at=NOW()
      WHERE id=$10
    ...
    for(const type of ['WAREHOUSE','MINE','SAWMILL','QUARRY']){
      const exists=await q(`SELECT 1 FROM buildings WHERE user_id=$1 AND type=$2 LIMIT 1`,[row.id,type]);
...
async function repairPlayerIfEmpty(id,client=pool){
  const u=(await q(`SELECT * FROM users WHERE id=$1 FOR UPDATE`,[id],client)).rows[0];
  if(!u) return false;

  // In the designed economy, a player can never have zero land.
  // Treat land <= 0 as an invalid/broken starter state and restore starter data.
  const invalid = Number(u.land || 0) <= 0 || [u.scoin,u.iron,u.wood,u.stone,u.energy,u.steel,u.machines].every(v=>num(v)===0);
  ...
  await q(`
    UPDATE users SET
      ...
      land=$8, mine_level=$9, mines=0, production_updated_at=NOW()
    WHERE id=$10
  `,[STARTER.scoin,STARTER.iron,STARTER.steel,STARTER.wood,STARTER.stone,STARTER.energy,STARTER.machines,STARTER.land,STARTER.mine_level,id],client);

  for(const type of ["WAREHOUSE","MINE","SAWMILL","QUARRY"]){
    const exists=await q(`SELECT 1 FROM buildings WHERE user_id=$1 AND type=$2 LIMIT 1`,[id,type],client);
    if(!exists.rows.length){
      await q(`INSERT INTO buildings(user_id,type,level) VALUES($1,$2,1)`,[id,type],client);
    }
  }
  return true;
}
...
async function gameState(id,client=pool){
  // Final safety net: repair an empty/broken profile immediately before any state is read.
  // This guarantees existing zeroed accounts recover even if they were created by an older version.
  await repairPlayerIfEmpty(id,client);
...
app.get("/health",async()=>({ok:true,service:"sanal-ekonomi",version:"19.0.0",currency:"SCoin"}));
...
app.post("/register",async(req,reply)=>{
  try{
    ...
    try{
      ...
      await q(`UPDATE users SET scoin=$1,iron=$2,steel=$3,wood=$4,stone=$5,energy=$6,machines=$7,land=$8,mine_level=$9,mines=0,production_updated_at=NOW() WHERE id=$10`,[STARTER.scoin,STARTER.iron,STARTER.steel,STARTER.wood,STARTER.stone,STARTER.energy,STARTER.machines,STARTER.land,STARTER.mine_level,id],client);
...
app.get("/me",{preHandler:auth},async(req,reply)=>{
  try{
    ...
    try{
      await c.query("BEGIN");
      await repairPlayerIfEmpty(req.user.id,c);
